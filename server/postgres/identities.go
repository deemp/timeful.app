...
func (r *Repository) CreateEventVisitorCredential(ctx context.Context, value *EventVisitorCredential) error {
	...
	return r.db.QueryRow(ctx, `INSERT INTO event_visitor_credentials (event_visitor_identity_id, credential_hash)
 VALUES ($1, $2) RETURNING id, created_at`, value.EventVisitorIdentityID, value.CredentialHash).Scan(&value.ID, &value.CreatedAt)
}
...
func (r *Repository) GetEventVisitorCredential(ctx context.Context, visitorID, credentialID string) (*EventVisitorCredential, error) {
	...
	err := r.db.QueryRow(ctx, `SELECT id, event_visitor_identity_id, credential_hash, created_at, revoked_at
 FROM event_visitor_credentials WHERE event_visitor_identity_id = $1 AND id::text = $2`, visitorID, credentialID).Scan(&value.ID, &value.EventVisitorIdentityID, &value.CredentialHash, &value.CreatedAt, &value.RevokedAt)
	return value, err
}
